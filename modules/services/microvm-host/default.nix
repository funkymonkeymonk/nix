...
in {
  ...
  # Use optionalAttrs for the platform check (evaluated against `options`, not `config`)
  # Use mkIf for the enable check (lazy - avoids infinite recursion with config)
  config = lib.optionalAttrs isNixOS (mkIf cfg.enable {
