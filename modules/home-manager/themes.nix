...
# (Applications/Ghostty.app/Contents/Resources/ghostty/themes/Earthsong)
{
  ...
  # Delta reads its theme from [delta] in ~/.gitconfig.  We expose an attrset
  # that callers pass to programs.git.extraConfig.
  deltaConfig = {
    delta = {
      syntax-theme = "Earthsong";
      side-by-side = false;
      line-numbers = true;
      navigate = true;
    };
  ...
  # -- alacritty ---------------------------------------------------------
  # Alacritty TOML colors section (v0.13+).
  alacrittyColors = {
    primary = {
      background = p.bg;
      foreground = p.fg;
