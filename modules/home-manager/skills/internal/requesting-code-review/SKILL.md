...
**1. Get commit IDs:**
```bash
BASE_SHA=$(jj log -r '@-' --no-graph -T 'commit_id')  # parent commit
HEAD_SHA=$(jj log -r '@' --no-graph -T 'commit_id')   # current commit
```
...
BASE_SHA=$(jj log -r 'description("Task 1")' --no-graph -T 'commit_id' | head -1)
