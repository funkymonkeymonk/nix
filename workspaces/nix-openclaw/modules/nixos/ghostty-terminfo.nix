...
# This compiles the terminfo source directly so it works on all platforms
# without depending on the ghostty package (which may not be available on Linux).
# See: https://github.com/ghostty-org/ghostty/discussions/5753
{
  pkgs,
  lib,
  ...
}: let
  # Build terminfo from source description — works on any platform with ncurses
  ghosttyTerminfo =
    pkgs.runCommand "ghostty-terminfo" {
      nativeBuildInputs = [pkgs.ncurses];
    } ''
      mkdir -p $out/share/terminfo
      cat > ghostty.terminfo << 'TERMINFO_SRC'
