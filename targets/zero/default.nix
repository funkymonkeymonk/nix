{
  ...
  # Sleep/hibernate policy and systemd services
  systemd = {
    sleep.extraConfig = ''
      AllowSuspend=no
      AllowHibernation=no
    ...
    services.tubearchivist-env = {
      description = "Create environment file for TubeArchivist container";
      wantedBy = ["docker-tubearchivist.service"];
      before = ["docker-tubearchivist.service"];
      serviceConfig = {
        Type = "oneshot";
      ...
      script = ''
        ...
        if [[ -f /run/secrets/tubearchivist-username && -f /run/secrets/tubearchivist-password ]]; then
          ...
          # Fallback to placeholder values if secrets are not available
          cat > /run/tubearchivist/environment << EOF
